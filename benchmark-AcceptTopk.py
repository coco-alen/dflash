...
def dflash_generate(
    ...
    output_ids[:, num_input_tokens:num_input_tokens+1] = sample(output.logits, temperature)[:,:,0]
    if block_size > 1:
    ...
    while start < max_length:
        ...
        posterior = sample(output.logits, temperature)
        _, seq_len, _ = output.logits.shape
        acceptance_length = 0
        if block_size > 1:
            for idx in range(seq_len-1):
                if torch.isin(block_output_ids[:, 1+idx], posterior[:, idx, :]):
                    acceptance_length += 1
                else:
                    break
        else:
            acceptance_length = (block_output_ids[:, 1:] == posterior[:, :-1, 0]).cumprod(dim=1).sum(dim=1)[0].item()
        
        output_ids[:, start : start + acceptance_length + 1] = block_output_ids[:, : acceptance_length + 1]
        output_ids[:, start + acceptance_length + 1] = posterior[:, acceptance_length, 0]
